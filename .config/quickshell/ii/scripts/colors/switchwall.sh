...
# New function to apply wallpaper with SWWW
apply_wallpaper_swww() {
    ...
    # Generate colors with wal in parallel
    wal -i "$imgpath" -q &
    local wal_pid=$!
    ...
    # Apply colors to Dolphin/KDE without restarting
    if [ -f ~/.cache/wal/colors-kde.conf ]; then
        cp ~/.cache/wal/colors-kde.conf ~/.config/kdeglobals
        # Apply changes without restarting applications
        if command -v qdbus > /dev/null; then
            # Reload KDE configuration
            qdbus org.kde.KWin /KWin reconfigure 2>/dev/null
            # Notify Qt applications of color changes
...
switch() {
    ...
    if [[ "$color_flag" == "1" ]]; then
        ...
        if [[ "$USE_SWWW" == "true" ]] && ! is_video "$imgpath"; then
            # Update path in config first
            #set_wallpaper_path "$imgpath"
            
            # Apply wallpaper with swww and pywal
            apply_wallpaper_swww "$imgpath"
            
            # Use the wallpaper to generate colors also with matugen (if enabled)
